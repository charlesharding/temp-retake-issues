...
bland_res_boot <- function(data, reps = 20000) {
  # Obtain Bland-Altman results in a convenient format, with 
  # temperature.2 - temperature.1.
  # This version of the function uses bootstrapping that approximately accounts 
  # for multiple measurement pairs per patient.
  ...
  res <- lapply(
    ...
    function(i) {
      ...
      g <- function(i){
        ...
        # x <- lapply(i, function(j) {
          # y <- w[w$uniquepid %in% p[j, ]$uniquepid, ]$dif
          # if(length(y) != 1){
        ...
        x_single <- do.call(c, lapply(x, `[[`, 1)) # With only 1 pair per patient
        x <- do.call(c, x) # With all pairs
